...
export default function MediaImage({ url, alt, className, mime, autoDownload = true }: { url: string; alt?: string; className?: string; mime?: string; autoDownload?: boolean }) {
  ...
  // Placeholder (either for GIF when autoplay off or when autoDownload is off)
  return (
    <button
      type="button"
      ...
      aria-label={gif ? 'Play GIF' : 'Load image'}
    >
      {gif && thumb && <img src={thumb} alt={alt || ''} className="absolute inset-0 w-full h-full object-contain" />}
      {gif && !thumb && (
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <span className="text-white text-2xl">{gif ? '►' : '⤓'}</span>
      </div>
    </button>
  );
}
